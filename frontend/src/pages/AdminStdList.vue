...
export default {
  ...
  methods: {
    async fetchStudents() {
      const url = `${import.meta.env.VITE_APP_BASE_URL}/api/students/std_list/${this.subjectCode}`;
      console.log(`Fetching students from: ${url}`); // log API URL
      try {
    ...
    async searchStudents() {
      ...
      const url = `${import.meta.env.VITE_APP_BASE_URL}/api/students/search?keyword=${this.searchKeyword}`;
      console.log(`Searching students from: ${url}`); // log API URL
      try {
        const response = await axios.get(url);
    ...
    async addStudentToSubject(student) {
      const url = `${import.meta.env.VITE_APP_BASE_URL}/api/students/subject-list`;
      console.log(`Adding student to subject from: ${url}`); // log API URL
      try {
    ...
    async removeStudent(studentId) {
      console.log("std_code for delete = "+studentId);
      const confirmed = window.confirm("คุณแน่ใจว่าต้องการลบรายชื่อนักศึกษานี้ใช่ไหม?");
      if (confirmed) {
        const url = `${import.meta.env.VITE_APP_BASE_URL}/api/students/students-list/${studentId}?sub_code=${this.subjectCode}`;
        console.log(`Removing student from: ${url}`); // log API URL
        try {
    ...
    async updateStudentStatus(student) {
      ...
      const url = `${import.meta.env.VITE_APP_BASE_URL}/api/students/subject-list/${student.std_code}`;
      console.log(`Updating student status from: ${url}`); // log API URL
      try {
        await axios.put(url, {
          status: newStatus
        });
